...
public final class TweetNaclFast {
  ...
  public static final class Box {
    ...
    public byte[] after(byte[] message, final int moff, final int mlen, byte[] theNonce) {
      ...
      if (0 != crypto_box_afternm(c, m, m.length, theNonce, sharedKey)) return null;
    ...
    public byte[] open_after(byte[] box, final int boxoff, final int boxlen, byte[] theNonce) {
      ...
      if (crypto_box_open_afternm(m, c, c.length, theNonce, sharedKey) != 0) return null;
  ...
  public static final class SecretBox {
    ...
    public byte[] box(byte[] message, final int moff, final int mlen, byte[] theNonce) {
      ...
      if (0 != crypto_secretbox(c, m, m.length, theNonce, key)) return null;
  ...
  public static final class Signature {
    ...
    public byte[] open(byte[] signedMessage, final int smoff, final int smlen) {
      ...
      if (0 != crypto_sign_open(tmp, -1, signedMessage, smoff, smlen, theirPublicKey)) return null;
    ...
    public boolean detached_verify(byte[] message, byte[] signature) {
      ...
      if (theirPublicKey.length != publicKeyLength) return false;
    ...
    public static KeyPair keyPair_fromSecretKey(byte[] secretKey) {
      ...
      // copy sk
      for (int i = 0; i < kp.getSecretKey().length; i++) sk[i] = secretKey[i];

      // copy pk from sk
      for (int i = 0; i < kp.getPublicKey().length; i++) pk[i] = secretKey[32 + i]; // hard-copy
  ...
  private static int crypto_stream_salsa20_xor(
      byte[] c, int cpos, byte[] m, int mpos, long b, byte[] n, byte[] k) {
  ...
  public static int crypto_stream_xor(
      byte[] c, int cpos, byte[] m, int mpos, long d, byte[] n, byte[] k) {
    byte[] s = new byte[32];
  ...
  private static int crypto_onetimeauth(
      byte[] out, final int outpos, byte[] m, final int mpos, int n, byte[] k) {
  ...
  private static int crypto_onetimeauth_verify(
      byte[] h, final int hoff, byte[] m, final int moff, int /*long*/ n, byte[] k) {
    byte[] x = new byte[16];
  ...
  private static void A(
      long[] o, final int ooff, long[] a, final int aoff, long[] b, final int boff) {
    int i;
  ...
  public static int crypto_box_open_afternm(
      byte[] m, byte[] c, int /*long*/ d, byte[] n, byte[] k) {
  ...
  public static int crypto_box_open(
      byte[] m, byte[] c, int /*long*/ d, byte[] n, byte[] y, byte[] x) {
    byte[] k = new byte[32];
    crypto_box_beforenm(k, y, x);
  ...
  private static void cswap(long[] p[], long[] q[], byte b) {
    ...
    for (i = 0; i < 4; i++) sel25519(p[i], 0, q[i], 0, b);
  }
  ...
  private static void scalarmult(long[] p[], long[] q[], byte[] s, final int soff) {
    ...
    /// String dbgt = "";
    /// for (int dbg = 0; dbg < p.length; dbg ++) for (int dd = 0; dd < p[dbg].length; dd ++) dbgt
    // += " "+p[dbg][dd];
  ...
  public static int crypto_sign(
    ...
    for (i = 0; i < 32; i++)
      for (j = 0; j < 32; j++) x[i + j] += (h[i] & 0xff) * (long) (d[j] & 0xff);
