...
/**
 * Indicates that an operation is not possible unless the {@link net.dv8tion.jda.core.AccountType
 * AccountType} matches the one provided in {@link #getRequiredType()}
 */
public class AccountTypeException extends RuntimeException {
   ...
   *
   * @param requiredType The required {@link net.dv8tion.jda.core.AccountType AccountType} for the
   *     operation
   */
  public AccountTypeException(AccountType requiredType) {
    this(
        requiredType,
        "The current AccountType is not valid for the attempted action. Required AccountType: "
            + requiredType);
  }
  ...
  public static void check(AccountType actualType, AccountType requiredType) {
    if (actualType != requiredType) throw new AccountTypeException(requiredType);
  }
}
