...
/**
 * The core of JDA. Acts as a registry system of JDA. All parts of the the API can be accessed
...
public interface JDA {
  ...
  enum Status {
    ...
    /**
     * JDA is populating internal objects. This process often takes the longest of all Statuses
     * (besides CONNECTED)
     */
    LOADING_SUBSYSTEMS(true),
    /**
     * JDA has finished loading everything, is receiving information from Discord and is firing
     * events.
     */
    CONNECTED(true),
    /**
     * JDA's main websocket has been disconnected. This <b>DOES NOT</b> mean JDA has shutdown
     * permanently. This is an in-between status. Most likely ATTEMPTING_TO_RECONNECT or
     * SHUTTING_DOWN/SHUTDOWN will soon follow.
     */
    DISCONNECTED,
    /**
     * JDA session has been added to {@link net.dv8tion.jda.core.utils.SessionController
     * SessionController} and is awaiting to be dequeued for reconnecting.
     */
    RECONNECT_QUEUED,
    /**
     * When trying to reconnect to Discord JDA encountered an issue, most likely related to a lack
     * of internet connection, and is waiting to try reconnecting again.
     */
    ...
    /**
     * JDA has received a shutdown request or has been disconnected from Discord and reconnect is
     * disabled, thus, JDA is in the process of shutting down
     */
    SHUTTING_DOWN,
    /**
     * JDA has finished shutting down and this instance can no longer be used to communicate with
     * the Discord servers.
     */
    SHUTDOWN,
    /**
     * While attempting to authenticate, Discord reported that the provided authentication
  ...
  class ShardInfo {
    ...
    /**
     * Represents the id of the shard of the current instance. <br>
     ...
     * {@link JDABuilder#useSharding(int, int)}. <br>
     * This <b>does not</b> query Discord to determine the total number of shards. <br>
     * This <b>does not</b> represent the amount of logged in shards. <br>
     * It strictly represents the integer value provided to discord representing the total amount of
     * shards that the developer indicated that it was going to use when initially starting JDA.
     *
     * @return The total of shards based on the total provided by the developer during JDA
     *     initialization.
     */
    ...
    /**
     * Provides a shortcut method for easily printing shard info. <br>
     * Format: "[# / #]" <br>
  ...
  /**
   * The time in milliseconds that discord took to respond to our last heartbeat <br>
  ...
  /**
   * Contains all {@code cf-ray} headers that JDA received in this session. <br>
   * These receive a new value whenever the WebSockedClient reconnects to the gateway.
   *
   * <p>This is useful to monitor cloudflare activity from the Discord Developer perspective. <br>
  ...
  /**
   * Receives all valid {@code _trace} lines that have been sent to us in this session. <br>
   ...
   *
   * <p>The default EventManager is {@link net.dv8tion.jda.core.hooks.InterfacedEventManager
   * InterfacedEventListener}. <br>
   * There is also an {@link net.dv8tion.jda.core.hooks.AnnotatedEventManager AnnotatedEventManager}
   * available.
   *
  ...
  /**
   * Adds all provided listeners to the event-listeners that will be used to handle events. This
   * uses the {@link net.dv8tion.jda.core.hooks.InterfacedEventManager InterfacedEventListener} by
   * default. To switch to the {@link net.dv8tion.jda.core.hooks.AnnotatedEventManager
   * AnnotatedEventManager}, use {@link #setEventManager(IEventManager)}.
   *
  ...
  /**
   * Constructs a new {@link net.dv8tion.jda.core.entities.Guild Guild} with the specified name <br>
   * Use the returned {@link net.dv8tion.jda.core.requests.restaction.GuildAction GuildAction} to
   * provide further details and settings for the resulting Guild!
   *
   * <p>This RestAction does not provide the resulting Guild! It will be in a following {@link
   * net.dv8tion.jda.core.events.guild.GuildJoinEvent GuildJoinEvent}.
   *
   ...
   *     <ul>
   *       <li>{@link net.dv8tion.jda.core.AccountType#CLIENT AccountType.CLIENT} and the account is
   *           in 100 or more guilds
   *       <li>{@link net.dv8tion.jda.core.AccountType#BOT AccountType.BOT} and the account is in 10
   *           or more guilds
   *     </ul>
   *
   * @throws java.lang.IllegalArgumentException If the provided name is empty, {@code null} or not
   *     between 2-100 characters
   * @return {@link net.dv8tion.jda.core.requests.restaction.GuildAction GuildAction} <br>
   *     Allows for setting various details for the resulting Guild
   */
  GuildAction createGuild(String name);

  /**
   * {@link net.dv8tion.jda.core.utils.cache.CacheView CacheView} of all cached {@link
   * net.dv8tion.jda.core.managers.AudioManager AudioManagers} created for this JDA instance. <br>
   * AudioManagers are created when first retrieved via {@link
   * net.dv8tion.jda.core.entities.Guild#getAudioManager() Guild.getAudioManager()}. <u>Using this
   * will perform better than calling {@code Guild.getAudioManager()} iteratively as that would
   * cause many useless audio managers to be created!</u>
   *
  ...
  /**
   * Immutable list of all created {@link net.dv8tion.jda.core.managers.AudioManager AudioManagers}
   ...
   * {@link net.dv8tion.jda.core.entities.User Users} visible to this JDA session.
   *
  ...
  /**
   * This unmodifiable returns all {@link net.dv8tion.jda.core.entities.User Users} that have the
   * same username as the one provided. <br>
   * If there are no {@link net.dv8tion.jda.core.entities.User Users} with the provided name, then
   * this returns an empty list.
   *
   * <p><b>Note: </b> This does **not** consider nicknames, it only considers {@link
   * net.dv8tion.jda.core.entities.User#getName()}
   *
   ...
   *     net.dv8tion.jda.core.entities.User#getName()}.
   * @return Possibly-empty list of {@link net.dv8tion.jda.core.entities.User Users} that all have
   *     the same name as the provided name.
   */
  ...
  /**
   * Attempts to retrieve a {@link net.dv8tion.jda.core.entities.User User} object based on the
   * provided id. <br>
   * This first calls {@link #getUserById(long)}, and if the return is {@code null} then a request
   * is made to the Discord servers.
   *
   * <p>The returned {@link net.dv8tion.jda.core.requests.RestAction RestAction} can encounter the
   * following Discord errors:
   *
   * <ul>
   *   <li>{@link net.dv8tion.jda.core.requests.ErrorResponse#UNKNOWN_USER
   *       ErrorResponse.UNKNOWN_USER} <br>
   *       Occurs when the provided id does not refer to a {@link net.dv8tion.jda.core.entities.User
   *       User} known by Discord. Typically occurs when developers provide an incomplete id (cut
   *       short).
   * </ul>
   *
   * @param id The id of the requested {@link net.dv8tion.jda.core.entities.User User}.
   * @throws net.dv8tion.jda.core.exceptions.AccountTypeException This endpoint is {@link
   *     AccountType#BOT} only.
   * @throws java.lang.NumberFormatException If the provided {@code id} cannot be parsed by {@link
   *     Long#parseLong(String)}
   * @throws java.lang.IllegalArgumentException
   *     <ul>
   *       <li>If the provided id String is null.
   *       <li>If the provided id String is empty.
   *     </ul>
   *
  ...
  /**
   * {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView} of all cached
   * {@link net.dv8tion.jda.core.entities.Guild Guilds} visible to this JDA session.
   *
   * @return {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView}
   */
   ...
   * then this list will only contain the {@link net.dv8tion.jda.core.entities.Guild Guilds} that
   * the shard is actually connected to. Discord determines which guilds a shard is connect to using
   * the following format: <br>
   * Guild connected if shardId == (guildId {@literal >>} 22) % totalShards; <br>
  ...
  /**
   * An unmodifiable list of all {@link net.dv8tion.jda.core.entities.Guild Guilds} that have the
   * same name as the one provided. <br>
   * If there are no {@link net.dv8tion.jda.core.entities.Guild Guilds} with the provided name, then
   * this returns an empty list.
   *
   * @param name The name of the requested {@link net.dv8tion.jda.core.entities.Guild Guilds}.
   * @param ignoreCase Whether to ignore case or not when comparing the provided name to each {@link
   *     net.dv8tion.jda.core.entities.Guild#getName()}.
   * @return Possibly-empty list of all the {@link net.dv8tion.jda.core.entities.Guild Guilds} that
   ...
   * cached {@link net.dv8tion.jda.core.entities.Role Roles} visible to this JDA session.
   *
   * @return Unified {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView}
   * @see net.dv8tion.jda.core.utils.cache.CacheView#allSnowflakes(java.util.function.Supplier)
   *     CacheView.allSnowflakes(...)
   */
  SnowflakeCacheView<Role> getRoleCache();

  /**
   * All {@link net.dv8tion.jda.core.entities.Role Roles} this JDA instance can see. <br>
  ...
  /**
   * {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView} of all cached
   * {@link net.dv8tion.jda.core.entities.Category Categories} visible to this JDA session.
   *
   * @return {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView}
   */
  ...
  /**
   * {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView} of all cached
   * {@link net.dv8tion.jda.core.entities.TextChannel TextChannels} visible to this JDA session.
   *
   * @return {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView}
   */
  ...
  /**
   * An unmodifiable list of all {@link net.dv8tion.jda.core.entities.TextChannel TextChannels} that
   ...
   * net.dv8tion.jda.core.Permission#MESSAGE_READ Permission.MESSAGE_READ} permission in.
   *
   * @param name The name of the requested {@link net.dv8tion.jda.core.entities.TextChannel
   *     TextChannels}.
   * @param ignoreCase Whether to ignore case or not when comparing the provided name to each {@link
   *     net.dv8tion.jda.core.entities.TextChannel#getName()}.
   * @return Possibly-empty list of all the {@link net.dv8tion.jda.core.entities.TextChannel
   *     TextChannels} that all have the same name as the provided name.
   */
  ...
  /**
   * {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView} of all cached
   * {@link net.dv8tion.jda.core.entities.VoiceChannel VoiceChannels} visible to this JDA session.
   *
   * @return {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView}
   */
  ...
  /**
   * An unmodifiable list of all {@link net.dv8tion.jda.core.entities.VoiceChannel VoiceChannels}
   ...
   * provided name, then this returns an empty list.
   *
   * @param name The name of the requested {@link net.dv8tion.jda.core.entities.VoiceChannel
   *     VoiceChannels}.
   * @param ignoreCase Whether to ignore case or not when comparing the provided name to each {@link
   *     net.dv8tion.jda.core.entities.VoiceChannel#getName()}.
   * @return Possibly-empty list of all the {@link net.dv8tion.jda.core.entities.VoiceChannel
   *     VoiceChannels} that all have the same name as the provided name.
   */
  ...
  /**
   * {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView} of all cached
   * {@link net.dv8tion.jda.core.entities.PrivateChannel PrivateChannels} visible to this JDA
   * session.
   *
   * @return {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView}
   */
  ...
  /**
   * Unified {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView} of all
   * cached {@link net.dv8tion.jda.core.entities.Emote Emotes} visible to this JDA session.
   *
   * @return Unified {@link net.dv8tion.jda.core.utils.cache.SnowflakeCacheView SnowflakeCacheView}
   * @see net.dv8tion.jda.core.utils.cache.CacheView#allSnowflakes(java.util.function.Supplier)
   *     CacheView.allSnowflakes(...)
  ...
  /**
   * Returns the currently logged in account represented by {@link
   * net.dv8tion.jda.core.entities.SelfUser SelfUser}. <br>
   * Account settings <b>cannot</b> be modified using this object. If you wish to modify account
   * settings please use the AccountManager which is accessible by {@link
   * net.dv8tion.jda.core.entities.SelfUser#getManager()} or {@link
   * net.dv8tion.jda.core.entities.SelfUser#getManagerUpdatable()}.
   *
   ...
   * <br>
   * Used to set {@link net.dv8tion.jda.core.entities.Game} and {@link
   * net.dv8tion.jda.core.OnlineStatus} information.
   *
   * @return The never-null {@link net.dv8tion.jda.core.managers.Presence Presence} for this
   *     session.
   */
   ...
   * Represents the information provided to {@link net.dv8tion.jda.core.JDABuilder#useSharding(int,
   * int)}.
   *
   * @return The shard information for this shard or {@code null} if this JDA instance isn't
   *     sharding.
   */
  ...
  /**
   * This value is the maximum amount of time, in seconds, that JDA will wait between reconnect
   * attempts. <br>
   * Can be set using {@link net.dv8tion.jda.core.JDABuilder#setMaxReconnectDelay(int)
  ...
  /**
   * Sets whether or not JDA should try to automatically reconnect if a connection-error is
   * encountered. <br>
   ...
   *
   * @return True if JDA will attempt to automatically reconnect when a connection-error is
   *     encountered.
   */
  ...
  /**
   * Used to determine if JDA will process MESSAGE_DELETE_BULK messages received from Discord as a
   * single {@link net.dv8tion.jda.core.events.message.MessageBulkDeleteEvent
   * MessageBulkDeleteEvent} or split the deleted messages up and fire multiple {@link
   * net.dv8tion.jda.core.events.message.MessageDeleteEvent MessageDeleteEvents}, one for each
   * deleted message.
   *
   * <p>By default, JDA will separate the bulk delete event into individual delete events, but this
   ...
   *
   * @return Whether or not JDA currently handles the BULK_MESSAGE_DELETE event by splitting it into
   *     individual MessageDeleteEvents or not.
   */
  boolean isBulkDeleteSplittingEnabled();

  /**
   * Shuts down this JDA instance, closing all its connections. After this command is issued the JDA
   * Instance can not be used anymore. Already enqueued {@link
   * net.dv8tion.jda.core.requests.RestAction RestActions} are still going to be executed.
   *
   * <p>If you want this instance to shutdown without executing, use {@link #shutdownNow()
   * shutdownNow()}
   *
   * @see #shutdownNow()
   */
  ...
  /**
   * Shuts down this JDA instance instantly, closing all its connections. After this command is
   * issued the JDA Instance can not be used anymore. This will also cancel all queued {@link
   * net.dv8tion.jda.core.requests.RestAction RestActions}.
   *
   * <p>If you want this instance to shutdown without cancelling enqueued RestActions use {@link
   * #shutdown() shutdown()}
   *
   * @see #shutdown()
   */
   ...
   *
   * @throws net.dv8tion.jda.core.exceptions.AccountTypeException Thrown if the currently logged in
   *     account is {@link net.dv8tion.jda.core.AccountType#CLIENT}
