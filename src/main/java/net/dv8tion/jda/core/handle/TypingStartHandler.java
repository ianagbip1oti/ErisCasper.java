...
public class TypingStartHandler extends SocketHandler {
  ...
  protected Long handleInternally(JSONObject content) {
    ...
    else if (channel instanceof Group) user = ((GroupImpl) channel).getUserMap().get(userId);
    else user = api.getUserMap().get(userId);

    if (user == null)
      return null; // Just like in the comment above, if for some reason we don't have the user for
    // some reason
    ...
    api.getEventManager()
        .handle(new UserTypingEvent(api, responseNumber, user, channel, timestamp));
    return null;
  }
}
