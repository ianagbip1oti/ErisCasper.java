...
public class MessageUpdateHandler extends SocketHandler {
  ...
  protected Long handleInternally(JSONObject content) {
    if (content.has("author")) {
      ...
      return null;
    } else return handleMessageEmbed(content);
  }
        ...
        case EntityBuilder.MISSING_CHANNEL:
          {
            ...
            api.getEventCache()
                .cache(
                    EventCache.Type.CHANNEL, channelId, () -> handle(responseNumber, allContent));
        ...
        case EntityBuilder.MISSING_USER:
          {
            ...
            api.getEventCache()
                .cache(EventCache.Type.USER, authorId, () -> handle(responseNumber, allContent));
            EventCache.LOG.debug(
    ...
    switch (message.getChannelType()) {
      ...
      default:
        WebSocketClient.LOG.warn(
            "Received a MESSAGE_UPDATE with a unknown MessageChannel ChannelType. JSON: {}",
            content);
        return null;
    }
      ...
      api.getEventManager()
          .handle(
              new PrivateMessageEmbedEvent(
                  api, responseNumber, messageId, (PrivateChannel) channel, embeds));
    } else {
      api.getEventManager()
          .handle(
              new GroupMessageEmbedEvent(api, responseNumber, messageId, (Group) channel, embeds));
    }
    // Combo event
    api.getEventManager()
        .handle(new MessageEmbedEvent(api, responseNumber, messageId, channel, embeds));
    return null;
  }
  ...
  public void handleCallMessage(JSONObject content) {
    ...
    // Could have a mapping of all users who were participants at one point or another during the
    // call
