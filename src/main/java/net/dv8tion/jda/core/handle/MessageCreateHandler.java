      ...
      case TEXT:
        {
          ...
          manager.handle(new GuildMessageReceivedEvent(api, responseNumber, message));
          break;
        }
      case PRIVATE:
        {
          ...
          channel.setLastMessageId(message.getIdLong());
          manager.handle(new PrivateMessageReceivedEvent(api, responseNumber, message));
          break;
        }
      case GROUP:
        {
          ...
          channel.setLastMessageId(message.getIdLong());
          manager.handle(new GroupMessageReceivedEvent(api, responseNumber, message));
          break;
        }
      default:
        WebSocketClient.LOG.warn(
            "Received a MESSAGE_CREATE with a unknown MessageChannel ChannelType. JSON: {}",
            content);
        return null;
    }

    // Combo event
    manager.handle(new MessageReceivedEvent(api, responseNumber, message));
    return null;
  }
}
