  ...
  protected Long handleInternally(JSONObject content) {
    ...
    if (member == null) {
      WebSocketClient.LOG.debug(
          "Received GUILD_MEMBER_REMOVE for a Member that does not exist in the specified Guild.");
      return null;
    }

    if (member
        .getVoiceState()
        .inVoiceChannel()) // If this user was in a VoiceChannel, fire VoiceLeaveEvent.
    {
    ...
    // The user also is not a friend of this account in the case that the logged in account is a
    // client account.
