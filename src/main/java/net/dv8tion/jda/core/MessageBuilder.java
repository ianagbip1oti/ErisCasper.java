...
/**
 * Builder system used to build {@link net.dv8tion.jda.core.entities.Message Messages}. <br>
 * Internally the builder uses a {@link java.lang.StringBuilder} to take advantage of the
 * efficiencies offered by the StringBuilder, and the methods provided by this class are a
 * combination of those offered by the StringBuilder and {@link String#format(String, Object...)}.
 *
 * @since 1.0
...
public class MessageBuilder implements Appendable {
  ...
  public MessageBuilder(Message message) {
    if (message != null) {
      ...
      if (embeds != null && !embeds.isEmpty()) embed = embeds.get(0);
    }
  }
  ...
  public MessageBuilder(EmbedBuilder builder) {
    if (builder != null) this.embed = builder.build();
  }
   ...
   * TTS stands for Text-To-Speech. When a TTS method is received by the Discord client, it is
   * vocalized so long as the user has not disabled TTS.
   *
  ...
  /**
   * Adds a {@link net.dv8tion.jda.core.entities.MessageEmbed} to the Message. Embeds can be built
   * using the {@link net.dv8tion.jda.core.EmbedBuilder} and offer specialized formatting.
   *
   ...
   * of the built message(s). It is recommended to have only 100% unique strings to validate
   * messages via this nonce. <br>
   * The nonce will be available from the resulting message via {@link
   * net.dv8tion.jda.core.entities.Message#getNonce() Message.getNonce()} in message received by
   * events and RestAction responses. <br>
   * When {@code null} is provided no nonce will be used.
   *
   * @param nonce Validation nonce string
   * @return The MessageBuilder instance. Useful for chaining.
   ...
   * @throws java.lang.IllegalArgumentException If the provided content exceeds {@link
   *     net.dv8tion.jda.core.entities.Message#MAX_CONTENT_LENGTH Message.MAX_CONTENT_LENGTH}
   * @return The MessageBuilder instance. Useful for chaining.
  ...
  public MessageBuilder setContent(String content) {
    if (content == null) {
      ...
      Checks.check(
          ...
          "Content length may not exceed %d!",
   ...
   * Typical usage would be providing an {@link net.dv8tion.jda.core.entities.IMentionable
   * IMentionable} like {@link net.dv8tion.jda.core.entities.User User} or {@link
  ...
  /**
   * Appends a String using the specified chat {@link net.dv8tion.jda.core.MessageBuilder.Formatting
   * Formatting(s)}.
   *
  ...
  public MessageBuilder append(CharSequence text, Formatting... format) {
    ...
    if (blockPresent) builder.append(Formatting.BLOCK.getTag());
  ...
  /**
   * This method is an extended form of {@link String#format(String, Object...)}. It allows for all
   * of the token replacement functionality that String.format(String, Object...) supports. <br>
   ...
   *
   * <ul>
   *   <li>{@link net.dv8tion.jda.core.entities.IMentionable IMentionable} <br>
   *       These will output their {@link net.dv8tion.jda.core.entities.IMentionable#getAsMention()
   *       getAsMention} by default, some implementations have alternatives such as {@link
   *       net.dv8tion.jda.core.entities.User User} and {@link
   *       net.dv8tion.jda.core.entities.TextChannel TextChannel}.
   *   <li>{@link net.dv8tion.jda.core.entities.MessageChannel MessageChannel} <br>
   *       All message channels format to {@code "#" + getName()} by default, TextChannel has
   *       special handling and uses the getAsMention output by default and the MessageChannel
   *       output as alternative ({@code #} flag).
   *   <li>{@link net.dv8tion.jda.core.entities.Message Message} <br>
   *       Messages by default output their {@link
   *       net.dv8tion.jda.core.entities.Message#getContentDisplay() getContentDisplay()} value and
   *       as alternative use the {@link net.dv8tion.jda.core.entities.Message#getContentRaw()
   *       getContentRaw()} value
   * </ul>
   *
   * <p>Example: <br>
   * If you placed the following code in an method handling a {@link
   * net.dv8tion.jda.core.events.message.MessageReceivedEvent MessageReceivedEvent} <br>
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * It would build a message that mentions the author and says that he is really cool!. If the
   * user's name was "Minn" and his discriminator "6688", it would say: <br>
   *
   * <pre>  "Minn#6688 is really cool!"</pre>
   *
   * <br>
   * Note that this uses the {@code #} flag to utilize the alternative format for {@link
   * net.dv8tion.jda.core.entities.User User}. <br>
   * By default it would fallback to {@link
   * net.dv8tion.jda.core.entities.IMentionable#getAsMention()}
   *
   * @param format a format string.
   * @param args an array objects that will be used to replace the tokens, they must be provided in
   *     the order that the tokens appear in the provided format string.
   * @throws java.lang.IllegalArgumentException If the provided format string is {@code null} or
   *     empty
   ...
   * Discord uses <a href="https://highlightjs.org/">Highlight.js</a> for its language highlighting
   * support. You can find out what specific languages are supported <a
   * href="https://github.com/isagalaev/highlight.js/tree/master/src/languages">here</a>.
   *
  ...
  /**
   * Returns the current length of the content that will be built into a {@link
   * net.dv8tion.jda.core.entities.Message Message} when {@link #build()} is called. <br>
   * If this value is {@code 0} (and there is no embed) or greater than {@code 2000} when {@link
   * #build()} is called, an exception will be raised as you cannot send an empty message to Discord
   * and Discord has a hard limit of 2000 characters per message.
   *
   * <p><b>Hint:</b> You can use {@link #build(int, int)} or {@link
   * #buildAll(net.dv8tion.jda.core.MessageBuilder.SplitPolicy...) buildAll(SplitPolicy...)} as
   * possible ways to deal with the 2000 character cap.
   *
  ...
  /**
   * Replaces the first substring that matches the target string with the specified replacement
   * string.
   *
   * @param target the sequence of char values to be replaced
   * @param replacement the replacement sequence of char values
  ...
  /**
   * Replaces the last substring that matches the target string with the specified replacement
   * string.
   *
   * @param target the sequence of char values to be replaced
   * @param replacement the replacement sequence of char values
   ...
   *
   * <p>Use this over {@link #stripMentions(Guild)} if {@link net.dv8tion.jda.core.entities.User
   * User} mentions should be replaced with their {@link
   * net.dv8tion.jda.core.entities.User#getName()} instead of their Nicknames.
   *
   * @param jda The JDA instance used to resolve the mentions.
   * @return The MessageBuilder instance. Useful for chaining.
   */
   ...
   * Removes all mentions and replaces them with the closest looking textual representation.
   *
   * <p>Use this over {@link #stripMentions(JDA)} if {@link net.dv8tion.jda.core.entities.User User}
   * mentions should be replaced with their nicknames in a specific guild based. <br>
   * Uses {@link net.dv8tion.jda.core.entities.Member#getEffectiveName()}
   *
   * @param guild the guild for {@link net.dv8tion.jda.core.entities.User User} mentions
   * @return The MessageBuilder instance. Useful for chaining.
   */
  ...
  /**
   * Removes all mentions of the specified types and replaces them with the closest looking textual
   * representation.
   *
   * <p>Use this over {@link #stripMentions(JDA, Message.MentionType...)} if {@link
   * net.dv8tion.jda.core.entities.User User} mentions should be replaced with their nicknames in a
   * specific guild based. <br>
   * Uses {@link net.dv8tion.jda.core.entities.Member#getEffectiveName()}
   *
   * @param guild the guild for {@link net.dv8tion.jda.core.entities.User User} mentions
   * @param types the {@link net.dv8tion.jda.core.entities.Message.MentionType MentionTypes} that
   *     should be stripped
   * @return The MessageBuilder instance. Useful for chaining.
   */
   ...
   * representation.
   *
   * <p>Use this over {@link #stripMentions(Guild, Message.MentionType...)} if {@link
   * net.dv8tion.jda.core.entities.User User} mentions should be replaced with their {@link
  ...
  /**
   * Returns the index within this string of the first occurrence of the specified substring between
   ...
   *     <ul>
   *       <li>If the {@code fromIndex} is outside of the range of {@code 0} to {@link #length()}
   *       <li>If the {@code endIndex} is outside of the range of {@code 0} to {@link #length()}
   *       <li>If the {@code fromIndex} is greater than {@code endIndex}
   *     </ul>
   *
   * @return the index of the first occurrence of the specified substring between the specified
   *     indices or {@code -1} if there is no such occurrence.
   */
  public int indexOf(CharSequence target, int fromIndex, int endIndex) {
    if (fromIndex < 0) throw new IndexOutOfBoundsException("index out of range: " + fromIndex);
    if (endIndex < 0) throw new IndexOutOfBoundsException("index out of range: " + endIndex);
    if (fromIndex > length()) throw new IndexOutOfBoundsException("fromIndex > length()");
    if (fromIndex > endIndex) throw new IndexOutOfBoundsException("fromIndex > endIndex");
  ...
  /**
   * Returns the index within this string of the last occurrence of the specified substring between
   * the specified indices.
   *
   * <p>If no such value of {@code target} exists, then {@code -1} is returned.
   *
   ...
   *     </ul>
   *
   * @return the index of the last occurrence of the specified substring between the specified
   *     indices or {@code -1} if there is no such occurrence.
   */
  ...
  /**
   * Creates a {@link net.dv8tion.jda.core.requests.restaction.MessageAction MessageAction} with the
   * current settings without building a {@link net.dv8tion.jda.core.entities.Message Message}
   * instance first.
   *
   * @param channel The not-null target {@link net.dv8tion.jda.core.entities.MessageChannel
   *     MessageChannel}
   ...
   *     does not have permission to send or read messages in this channel, or if this is a
   ...
   *           {@link net.dv8tion.jda.core.entities.MessageEmbed} was provided to {@link
   *           #setEmbed(net.dv8tion.jda.core.entities.MessageEmbed)})
   *       <li>If you attempt to build() a Message with more than 2000 characters of content.
   *     </ul>
   *
  ...
  /**
   * Creates a {@link java.util.Queue Queue} of {@link net.dv8tion.jda.core.entities.Message
   * Message} objects from this MessageBuilder.
   *
   * <p>This method splits the content if it exceeds 2000 chars. The splitting behaviour can be
   * customized using {@link SplitPolicy SplitPolicies}. The method will try the policies in the
   * order they are passed to it. <br>
   * If no SplitPolicy is provided each message will be split after exactly 2000 chars.
   *
   * <p><b>This is not Markdown safe.</b> An easy workaround is to include <a
   * href="https://en.wikipedia.org/wiki/Zero-width_space">Zero Width Spaces</a> as predetermined
   * breaking points to the message and only split on them.
   *
   * @param policy The {@link net.dv8tion.jda.core.MessageBuilder.SplitPolicy} defining how to split
   *     the text in the MessageBuilder into different, individual messages.
  ...
  /**
   * Interface to allow custom implementation of Splitting rules for {@link
   * #buildAll(net.dv8tion.jda.core.MessageBuilder.SplitPolicy...)
  ...
  public interface SplitPolicy {
      ...
      public int nextMessage(final int currentBeginIndex, final MessageBuilder builder) {
        int currentEndIndex =
            builder.lastIndexOf(
                this.chars,
                currentBeginIndex,
                currentBeginIndex + 2000 - (this.remove ? this.chars.length() : 0));
     ...
     *
     * @param currentBeginIndex the index the next {@link net.dv8tion.jda.core.entities.Message
     *     Message} should start from
     * @param builder the {@link net.dv8tion.jda.core.MessageBuilder MessageBuilder}
  ...
  /**
   * Holds the available formatting used in {@link MessageBuilder#append(java.lang.CharSequence,
