...
/**
 * The Presence associated with the provided JDA instance <br>
 * <b>Note that this does not automatically handle the 5/60 second rate limit!</b>
 *
 * @since 3.0
...
public class PresenceImpl implements Presence {
  ...
  public void setPresence(OnlineStatus status, Game game, boolean idle) {
    ...
    if (gameObj == null) object.put("game", JSONObject.NULL);
    else object.put("game", gameObj);
    object.put("afk", idle);
  ...
  private JSONObject getGameJson(Game game) {
    if (game == null || game.getName() == null || game.getType() == null) return null;
    JSONObject gameObj = new JSONObject();
    gameObj.put("name", game.getName());
    gameObj.put("type", game.getType().getKey());
    if (game.getUrl() != null) gameObj.put("url", game.getUrl());
  ...
  protected void update(JSONObject data) {
    api.getClient()
        .send(new JSONObject().put("d", data).put("op", WebSocketCode.PRESENCE).toString());
  }
}
