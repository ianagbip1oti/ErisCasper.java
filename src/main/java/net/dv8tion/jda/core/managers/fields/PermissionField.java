...
/**
 * Specification Manager Field for a {@link net.dv8tion.jda.core.Permission Permission} depending
 * Field.
 *
 * <p>The specification for this Field will automatically check provided {@link
 * net.dv8tion.jda.core.Permission Permissions} for access by using {@link
 * net.dv8tion.jda.core.utils.PermissionUtil#checkPermission(Member, Permission...)
 * PermissionUtil.checkPermission(Member, Permission...)} on the current account's Member instance
 * for the specified {@link net.dv8tion.jda.core.entities.Guild Guild}.
 *
 * <p><b>This class is an extension of {@link net.dv8tion.jda.core.managers.fields.RoleField
 * RoleField}</b> <br>
...
public class PermissionField extends RoleField<Long> {
  ...
  /**
   * Sets the value which should be used in the update operation for the Manager instance. <br>
   * This will cause {@link #isSet()} to return {@code true}!
   *
  ...
  /**
   * Sets the permissions for this PermissionField. <br>
   * Convenience method to provide multiple permissions with a single method.
   *
   * @param permissions The {@link net.dv8tion.jda.core.Permission Permissions} to use
   * @throws IllegalArgumentException If the provided permission collection or any of the
   *     permissions within it are null
   * @throws net.dv8tion.jda.core.exceptions.InsufficientPermissionException If the permissions
   *     provided require other permissions to be available
   * @return The {@link net.dv8tion.jda.core.managers.RoleManagerUpdatable RoleManagerUpdatable}
   *     instance for this PermissionField for chaining convenience
   */
  ...
  public RoleManagerUpdatable setPermissions(Collection<Permission> permissions) {
    ...
    permissions.forEach(p -> Checks.notNull(p, "Permission in the Collection"));
  ...
  public void checkValue(Long value) {
    ...
    Permission.getPermissions(value)
        .forEach(
            p -> {
              checkPermission(p);
            });
  ...
  /**
   * Adds the specified permissions to the result value <br>
   * If any of the specified permissions is present in the revoked permissions it will be removed!
   * <br>
   * <b>This does not apply immediately - it is applied in the value returned by {@link
   * #getValue()}</b>
   *
   * @param permissions Permissions that should be granted
  ...
  /**
   * Adds the specified permissions to the result value <br>
   * These will override permissions that are given through {@link #givePermissions(Collection)} and
   * {@link #givePermissions(Permission...)}! <br>
   * <b>This does not apply immediately - it is applied in the value returned by {@link
   * #getValue()}</b>
   *
   * @param permissions Permissions that should be revoked
   * @throws IllegalArgumentException If any of the provided Permissions is {@code null}
   * @return The {@link net.dv8tion.jda.core.managers.RoleManagerUpdatable RoleManagerUpdatable}
   *     instance for this PermissionField for chaining convenience
   */
  ...
  public Long getValue() {
    ...
    perms = value;
      ...
      perms = getOriginalValue(); // Otherwise, assume we are adding and removing from the original
    // value;
  ...
  /**
   * An immutable list of {@link net.dv8tion.jda.core.Permission Permissions} that are calculated
   * from {@link #getValue()} using {@link Permission#getPermissions(long)}
   *
  ...
  /**
   * An immutable list of {@link net.dv8tion.jda.core.Permission Permissions} that have been
   * calculated {@link #getOriginalValue()} using {@link Permission#getPermissions(long)}
   *
  ...
  protected void checkPermission(Permission perm) {
    if (!manager.getGuild().getSelfMember().hasPermission(perm))
      throw new InsufficientPermissionException(
          perm,
          "Cannot give / revoke the permission because the logged in account does not have access to it! Permission: "
              + perm);
  }
}
