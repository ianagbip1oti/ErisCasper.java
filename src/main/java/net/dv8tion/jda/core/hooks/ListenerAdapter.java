...
/**
 * An abstract implementation of {@link net.dv8tion.jda.core.hooks.EventListener EventListener}
 * which divides {@link net.dv8tion.jda.core.events.Event Events} for you.
 *
 * <p><b>Example:</b> <br>
 *
 * <pre><code>
...
public abstract class ListenerAdapter implements EventListener {
  ...
  public final void onEvent(Event event) {
    ...
    else if (event instanceof ResumedEvent) onResume((ResumedEvent) event);
    else if (event instanceof ReconnectedEvent) onReconnect((ReconnectedEvent) event);
    else if (event instanceof DisconnectEvent) onDisconnect((DisconnectEvent) event);
    else if (event instanceof ShutdownEvent) onShutdown((ShutdownEvent) event);
    else if (event instanceof StatusChangeEvent) onStatusChange((StatusChangeEvent) event);
    else if (event instanceof ExceptionEvent) onException((ExceptionEvent) event);
    ...
    // Combined Message Events (Combines Guild and Private message into 1 event)
    else if (event instanceof MessageReceivedEvent) onMessageReceived((MessageReceivedEvent) event);
    ...
    else if (event instanceof MessageEmbedEvent) onMessageEmbed((MessageEmbedEvent) event);
    ...
    // User Events
    else if (event instanceof UserNameUpdateEvent) onUserNameUpdate((UserNameUpdateEvent) event);
    else if (event instanceof UserAvatarUpdateEvent)
      onUserAvatarUpdate((UserAvatarUpdateEvent) event);
    else if (event instanceof UserGameUpdateEvent) onUserGameUpdate((UserGameUpdateEvent) event);
    else if (event instanceof UserOnlineStatusUpdateEvent)
      onUserOnlineStatusUpdate((UserOnlineStatusUpdateEvent) event);
    else if (event instanceof UserTypingEvent) onUserTyping((UserTypingEvent) event);

    // Self Events
    else if (event instanceof SelfUpdateAvatarEvent)
      onSelfUpdateAvatar((SelfUpdateAvatarEvent) event);
    else if (event instanceof SelfUpdateEmailEvent) onSelfUpdateEmail((SelfUpdateEmailEvent) event);
    else if (event instanceof SelfUpdateMFAEvent) onSelfUpdateMFA((SelfUpdateMFAEvent) event);
    else if (event instanceof SelfUpdateNameEvent) onSelfUpdateName((SelfUpdateNameEvent) event);
    ...
    else if (event instanceof GuildAvailableEvent) onGuildAvailable((GuildAvailableEvent) event);
    ...
    else if (event instanceof GuildUpdateIconEvent) onGuildUpdateIcon((GuildUpdateIconEvent) event);
    else if (event instanceof GuildUpdateMFALevelEvent)
      onGuildUpdateMFALevel((GuildUpdateMFALevelEvent) event);
    else if (event instanceof GuildUpdateNameEvent) onGuildUpdateName((GuildUpdateNameEvent) event);
    ...
    // Guild Voice Events
    else if (event instanceof GuildVoiceJoinEvent) onGuildVoiceJoin((GuildVoiceJoinEvent) event);
    else if (event instanceof GuildVoiceMoveEvent) onGuildVoiceMove((GuildVoiceMoveEvent) event);
    else if (event instanceof GuildVoiceLeaveEvent) onGuildVoiceLeave((GuildVoiceLeaveEvent) event);
    else if (event instanceof GuildVoiceMuteEvent) onGuildVoiceMute((GuildVoiceMuteEvent) event);
    ...
    else if (event instanceof RoleUpdateNameEvent) onRoleUpdateName(((RoleUpdateNameEvent) event));
    ...
    // Emote Events
    else if (event instanceof EmoteAddedEvent) onEmoteAdded((EmoteAddedEvent) event);
    else if (event instanceof EmoteRemovedEvent) onEmoteRemoved((EmoteRemovedEvent) event);

    // Emote Update Events
    else if (event instanceof EmoteUpdateNameEvent) onEmoteUpdateName((EmoteUpdateNameEvent) event);
    else if (event instanceof EmoteUpdateRolesEvent)
      onEmoteUpdateRoles((EmoteUpdateRolesEvent) event);

    // Debug Events
    else if (event instanceof HttpRequestEvent) onHttpRequest((HttpRequestEvent) event);

    // Generic subclasses - combining multiple events
    if (event instanceof GuildVoiceUpdateEvent) onGuildVoiceUpdate((GuildVoiceUpdateEvent) event);
    ...
    if (event instanceof GenericMessageEvent) onGenericMessage((GenericMessageEvent) event);
    ...
    else if (event instanceof GenericUserEvent) onGenericUser((GenericUserEvent) event);
    ...
    else if (event instanceof GenericCategoryEvent) onGenericCategory((GenericCategoryEvent) event);
    else if (event instanceof GenericRoleEvent) onGenericRole((GenericRoleEvent) event);
    else if (event instanceof GenericEmoteEvent) onGenericEmote((GenericEmoteEvent) event);

    // Generic events that have 2 levels of generic subclasses
    if (event instanceof GenericGuildEvent) onGenericGuild((GenericGuildEvent) event);

    if (event.getJDA().getAccountType() == AccountType.CLIENT) {
      // Relationship Events
      if (event instanceof FriendAddedEvent) onFriendAdded((FriendAddedEvent) event);
      else if (event instanceof FriendRemovedEvent) onFriendRemoved((FriendRemovedEvent) event);
      else if (event instanceof UserBlockedEvent) onUserBlocked((UserBlockedEvent) event);
      else if (event instanceof UserUnblockedEvent) onUserUnblocked((UserUnblockedEvent) event);
      ...
      // Group Events
      else if (event instanceof GroupJoinEvent) onGroupJoin((GroupJoinEvent) event);
      else if (event instanceof GroupLeaveEvent) onGroupLeave((GroupLeaveEvent) event);
      else if (event instanceof GroupUserJoinEvent) onGroupUserJoin((GroupUserJoinEvent) event);
      else if (event instanceof GroupUserLeaveEvent) onGroupUserLeave((GroupUserLeaveEvent) event);
      ...
      // Call Events
      else if (event instanceof CallCreateEvent) onCallCreate((CallCreateEvent) event);
      else if (event instanceof CallDeleteEvent) onCallDelete((CallDeleteEvent) event);
      ...
      // Call Voice Events
      else if (event instanceof CallVoiceJoinEvent) onCallVoiceJoin((CallVoiceJoinEvent) event);
      else if (event instanceof CallVoiceLeaveEvent) onCallVoiceLeave((CallVoiceLeaveEvent) event);
      ...
      else if (event instanceof GenericGroupEvent) onGenericGroup((GenericGroupEvent) event);
      else if (event instanceof GenericCallEvent) onGenericCall((GenericCallEvent) event);
    }
  }
}
