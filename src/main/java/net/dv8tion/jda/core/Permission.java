...
public enum Permission {
  ...
  public static final long ALL_CHANNEL_PERMISSIONS =
      Permission.getRaw(
          Arrays.stream(values()).filter(Permission::isChannel).collect(Collectors.toList()));

  /** All Guild specific permissions which are only available to roles */
  public static final long ALL_GUILD_PERMISSIONS =
      Permission.getRaw(
          Arrays.stream(values()).filter(Permission::isGuild).collect(Collectors.toList()));

  /**
   * All text channel specific permissions which are only available in text channel permission
   * overrides
   */
  public static final long ALL_TEXT_PERMISSIONS =
      Permission.getRaw(
          Arrays.stream(values()).filter(Permission::isText).collect(Collectors.toList()));

  /**
   * All voice channel specific permissions which are only available in voice channel permission
   * overrides
   */
  public static final long ALL_VOICE_PERMISSIONS =
      Permission.getRaw(
          Arrays.stream(values()).filter(Permission::isVoice).collect(Collectors.toList()));
   ...
   * For more information about Discord's offset system refer to <a
   * href="https://discordapi.readthedocs.org/en/latest/reference/channels/permissions.html#permissions-number">Discord
   * Permission Numbers</a>.
   *
   ...
   * This is equivalent to: <code>1 {@literal <<} {@link #getOffset()}</code>
   *
  ...
  /**
   * Returns whether or not this Permission is present at the Guild level (configurable via {@link
   * net.dv8tion.jda.core.entities.Role Roles})
   *
  ...
  /**
   * Returns whether or not this Permission is present Channel level (configurable via {@link
   * net.dv8tion.jda.core.entities.PermissionOverride PermissionsOverrides})
   *
  ...
  /**
   * Whether this permission is specifically for {@link net.dv8tion.jda.core.entities.TextChannel
   * TextChannels}
   *
  ...
  /**
   * Whether this permission is specifically for {@link net.dv8tion.jda.core.entities.VoiceChannel
   * VoiceChannels}
   *
  ...
  /**
   * A list of all {@link net.dv8tion.jda.core.Permission Permissions} that are specified by this
   * raw long representation of permissions. The is best used with the getRaw methods in {@link
   * net.dv8tion.jda.core.entities.Role Role}, {@link
   * net.dv8tion.jda.core.entities.PermissionOverride PermissionOverride} or {@link
   * net.dv8tion.jda.core.utils.PermissionUtil}.
   *
   * <p>Examples: <br>
   * {@link net.dv8tion.jda.core.entities.Role#getPermissionsRaw() Role.getPermissionsRaw()} <br>
   * {@link
   * net.dv8tion.jda.core.utils.PermissionUtil#getEffectivePermission(net.dv8tion.jda.core.entities.Channel,
   * net.dv8tion.jda.core.entities.Member) PermissionUtil.getEffectivePermission(Channel, Member)}
   *
  ...
  public static List<Permission> getPermissions(long permissions) {
    ...
    for (Permission perm : Permission.values()) {
      if (perm.equals(Permission.UNKNOWN)) continue;
  ...
  /**
   * This is effectively the opposite of {@link #getPermissions(long)}, this takes 1 or more {@link
   * net.dv8tion.jda.core.Permission Permissions} and returns the raw offset {@code long}
  ...
  /**
   * This is effectively the opposite of {@link #getPermissions(long)}, this takes a Collection of
   * {@link net.dv8tion.jda.core.Permission Permissions} and returns the raw offset {@code long}
   * representation of the permissions.
   *
   * @param permissions The Collection of permissions of which to form into the raw long
   *     representation.
   * @return Unsigned long representing the provided permissions.
   */
