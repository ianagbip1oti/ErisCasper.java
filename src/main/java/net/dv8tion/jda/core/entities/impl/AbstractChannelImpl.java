...
public abstract class AbstractChannelImpl<T extends AbstractChannelImpl<T>> implements Channel {
  ...
  public ChannelManager getManager() {
    ...
    if (mng == null) {
      synchronized (mngLock) {
        mng = manager;
        if (mng == null) mng = manager = new ChannelManager(this);
      }
    }
    return mng;
  }
  ...
  public ChannelManagerUpdatable getManagerUpdatable() {
    ...
    if (mng == null) {
      synchronized (mngLock) {
        ...
        if (mng == null) mng = managerUpdatable = new ChannelManagerUpdatable(this);
      }
    }
    return mng;
  }
  ...
  public PermissionOverrideAction createPermissionOverride(Member member) {
    ...
    if (!guild.equals(member.getGuild()))
      throw new IllegalArgumentException(
          "Provided member is not from the same guild as this channel!");
    if (overrides.containsKey(member.getUser().getIdLong()))
      throw new IllegalStateException(
          "Provided member already has a PermissionOverride in this channel!");

    Route.CompiledRoute route =
        Route.Channels.CREATE_PERM_OVERRIDE.compile(getId(), member.getUser().getId());
  ...
  public PermissionOverrideAction createPermissionOverride(Role role) {
    ...
    if (!guild.equals(role.getGuild()))
      throw new IllegalArgumentException(
          "Provided role is not from the same guild as this channel!");
    if (overrides.containsKey(role.getIdLong()))
      throw new IllegalStateException(
          "Provided role already has a PermissionOverride in this channel!");
  ...
  protected void checkPermission(Permission permission, String message) {
    if (!guild.getSelfMember().hasPermission(this, permission)) {
      if (message != null) throw new InsufficientPermissionException(permission, message);
