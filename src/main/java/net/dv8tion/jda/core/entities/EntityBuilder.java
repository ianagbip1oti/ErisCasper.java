...
public class EntityBuilder {
  ...
  public void createGuildFirstPass(JSONObject guild, Consumer<Guild> secondPassCallback) {
    ...
    if (Helpers.optBoolean(guild, "unavailable")) {
      ...
      // This is used for when GuildCreateHandler receives a guild that is currently unavailable.
      // During normal READY
      // loading for bots (which unavailable is always true) the secondPassCallback parameter will
      // always
      // be null.
      if (secondPassCallback != null) secondPassCallback.accept(guildObj);
    ...
    //
    // Either way, we now have enough information to fill in the general information about the
    // Guild.
    // This does NOT necessarily mean that we have all information to complete the guild.
    // For Client accounts, we will also need to use op 12 (GUILD_SYNC) to get all presences of
    // online users because
    // discord only provides Online users that we have an open PM channel with or are friends with
    // for Client accounts.
    // On larger guilds we will still need to request all users using op 8 (GUILD_MEMBERS_CHUNK).
    //
    // The code below takes the information we -do- have and starts to fill in the Guild. It won't
    // create anything
    ...
    guildObj
        ...
        .setDefaultNotificationLevel(
            Guild.NotificationLevel.fromKey(guild.getInt("default_message_notifications")))
        .setRequiredMFALevel(Guild.MFALevel.fromKey(guild.getInt("mfa_level")))
        .setExplicitContentLevel(
            Guild.ExplicitContentLevel.fromKey(guild.getInt("explicit_content_filter")));

    if (guild.isNull("features")) guildObj.setFeatures(Collections.emptySet());
    else {
    ...
    if (!guild.isNull("emojis")) {
      ...
      for (int i = 0; i < array.length(); i++) {
        ...
        JSONArray emoteRoles =
            object.isNull("roles") ? new JSONArray() : object.getJSONArray("roles");
    ...
    if (owner != null) guildObj.setOwner(owner);
    ...
    if (guild.has("presences")) {
      ...
      for (int i = 0; i < presences.length(); i++) {
        ...
        if (member == null)
          LOG.debug(
              "Received a ghost presence in GuildFirstPass! UserId: {} Guild: {}",
              userId,
              guildObj);
        else createPresence(member, presence);
      }
    }
    ...
    if (guild.has("channels")) {
      ...
      for (int i = 0; i < channels.length(); i++) {
        ...
        switch (type) {
          ...
          default:
            LOG.error(
                "Received a channel for a guild that isn't a text, voice or category channel. JSON: {}",
                channel);
        }
      }
    }
    ...
    if (!guild.isNull("system_channel_id"))
      guildObj.setSystemChannel(
          guildObj.getTextChannelsMap().get(guild.getLong("system_channel_id")));
    ...
    //    1) If this is a Bot account, immediately call EntityBuilder#createGuildSecondPass, thus
    // finishing
    //        the Guild object creation process.
    //    2) If this is a Client account, it will request op 12 (GUILD_SYNC) to make sure we have
    // all information
    //        about online users as GUILD_MEMBERS_CHUNK does not include presence information, and
    // when loading the
    //        members from GUILD_MEMBERS_CHUNK, we assume they are offline. GUILD_SYNC makes sure
    // that we mark them
    //        properly. After GUILD_SYNC is received by GuildSyncHandler, it will call
    // EntityBuilder#createGuildSecondPass
    //
    // If we actually -did- get all of the users needed, then we don't need to Chunk. Furthermore,
    // we don't need to use GUILD_SYNC because we always get presences with users thus we have all
    // information
    ...
    if (guild.getJSONArray("members").length() != guild.getInt("member_count")) {
      ...
      // If we are already past READY / RESUME, then chunk at runtime. Otherwise, pass back to the
      // ReadyHandler
      ...
      if (api.getClient().isReady()) {
        ...
        JSONObject obj =
            new JSONObject()
                ...
                .put("d", new JSONObject().put("guild_id", id).put("query", "").put("limit", 0));
        api.getClient().chunkOrSyncRequest(obj);
      } else {
    ...
    // As detailed in the comment above, if we've made it this far then we have all member
    // information needed to
    // create the Guild. Thus, we fill in the remaining information, unlock the guild, and provide
    // the guild
    // to the callback
  ...
  public void createGuildSecondPass(long guildId, List<JSONArray> memberChunks) {
    ...
    if (guildObj == null)
      throw new IllegalStateException(
          "Attempted to perform a second pass on an unknown Guild. Guild not in JDA "
              + "mapping. GuildId: "
              + guildId);
    if (guildJson == null)
      throw new IllegalStateException(
          "Attempted to perform a second pass on an unknown Guild. No cached Guild "
              + "for second pass. GuildId: "
              + guildId);
    if (secondPassCallback == null)
      throw new IllegalArgumentException("No callback provided for the second pass on the Guild!");

    for (JSONArray chunk : memberChunks) createGuildMemberPass(guildObj, chunk);
    ...
    if (guildObj.getOwner() == null)
      LOG.error(
          "Never set the Owner of the Guild: {} because we don't have the owner User object! How?!",
          guildObj.getId());
  ...
  public void handleGuildSync(GuildImpl guild, JSONArray members, JSONArray presences) {
    ...
    for (int i = 0; i < presences.length(); i++) {
      ...
      else this.createPresence(member, presenceJson);
    }
  }
  ...
  private void createGuildChannelPass(GuildImpl guildObj, JSONArray channels) {
    for (int i = 0; i < channels.length(); i++) {
      ...
      switch (type) {
        ...
        default:
          LOG.error(
              "Received a channel for a guild that isn't a text, voice or category channel (ChannelPass). JSON: {}",
              channel);
      }
      ...
      if (channelObj != null) {
        ...
        LOG.error(
            "Got permission_override for unknown channel with id: {}", channel.getString("id"));
      }
    }
  }
  ...
  public void createGuildVoiceStatePass(GuildImpl guildObj, JSONArray voiceStates) {
    for (int i = 0; i < voiceStates.length(); i++) {
      ...
      if (member == null) {
        LOG.error(
            "Received a VoiceState for a unknown Member! GuildId: "
                + guildObj.getId()
                + " MemberId: "
                + voiceStateJson.getString("user_id"));
        continue;
      }
      ...
      if (voiceChannel != null)
        ...
        LOG.error(
            "Received a GuildVoiceState with a channel ID for a non-existent channel! ChannelId: {} GuildId: {} UserId: {}",
            channelId,
            guildObj.getId(),
            userId);
      ...
      voiceState
          .setSelfMuted(Helpers.optBoolean(voiceStateJson, "self_mute"))
  ...
  private User createUser(JSONObject user, boolean fake, boolean modifyCache) {
    ...
    if (userObj == null) {
      ...
      if (userObj != null) {
        ...
        if (modifyCache) {
          if (fake) api.getFakeUserMap().put(id, userObj);
          else api.getUserMap().put(id, userObj);
        }
      }
    }
  ...
  public Member createMember(GuildImpl guild, JSONObject memberJson) {
    ...
    member
        .setJoinDate(OffsetDateTime.parse(memberJson.getString("joined_at")))
    ...
    for (int k = 0; k < rolesJson.length(); k++) {
      ...
      if (r == null) {
        LOG.debug(
            "Received a Member with an unknown Role. MemberId: {} GuildId: {} roleId: {}",
  ...
  public void createPresence(Object memberOrFriend, JSONObject presenceJson) {
    if (memberOrFriend == null) throw new NullPointerException("Provided memberOrFriend was null!");
    ...
    if (gameJson != null && !gameJson.isNull("name")) {
      try {
        ...
        String userId;
        if (memberOrFriend instanceof Member) userId = ((Member) memberOrFriend).getUser().getId();
  ...
  public static Game createGame(JSONObject gameJson) {
    ...
    if (!gameJson.isNull("assets")) {
      ...
      if (!assets.isNull("small_image")) {
        ...
        smallImageText =
            assets.isNull("small_text") ? null : String.valueOf(assets.get("small_text"));
      }
      if (!assets.isNull("large_image")) {
        largeImageKey = String.valueOf(assets.get("large_image"));
        largeImageText =
            assets.isNull("large_text") ? null : String.valueOf(assets.get("large_text"));
      }
    }
  ...
  public Category createCategory(JSONObject json, long guildId, boolean guildIsLoaded) {
    ...
    return channel.setName(json.getString("name")).setRawPosition(json.getInt("position"));
  }
  ...
  public PrivateChannel createPrivateChannel(JSONObject privatechat) {
    ...
    if (user
        == null) { // The API can give us private channels connected to Users that we can no longer
      // communicate with.
    ...
    PrivateChannelImpl priv =
        new PrivateChannelImpl(channelId, user)
    ...
    } else api.getPrivateChannelMap().put(channelId, priv);
    return priv;
  }
  ...
  public Message createMessage(
      JSONObject jsonObject, MessageChannel chan, boolean exceptionOnMissingUser) {
    ...
    final OffsetDateTime editTime =
        jsonObject.isNull("edited_timestamp")
            ? null
    ...
    final List<MessageReaction> reactions =
        map(jsonObject, "reactions", (obj) -> createMessageReaction(chan, id, obj));

    User user;
    switch (chan.getType()) {
      case PRIVATE:
        if (authorId == api.getSelfUser().getIdLong()) user = api.getSelfUser();
        else user = ((PrivateChannel) chan).getUser();
        break;
      case GROUP:
        user = api.getUserById(authorId);
        if (user == null) user = api.getFakeUserMap().get(authorId);
        if (user == null && fromWebhook) user = createFakeUser(author, false);
        if (user == null) {
          if (exceptionOnMissingUser)
            throw new IllegalArgumentException(MISSING_USER); // Specifically for MESSAGE_CREATE
          else user = createFakeUser(author, false); // Any other message creation
        }
      ...
      case TEXT:
        ...
        if (user == null) {
          if (fromWebhook || !exceptionOnMissingUser) user = createFakeUser(author, false);
          else throw new IllegalArgumentException(MISSING_USER); // Specifically for MESSAGE_CREATE
        }
        break;
      default:
        throw new IllegalArgumentException(
            "Invalid Channel for creating a Message [" + chan.getType() + ']');
    }
  ...
  public MessageReaction createMessageReaction(MessageChannel chan, long id, JSONObject obj) {
    ...
    if (emojiID != null) {
      ...
      if (emote == null) emote = new EmoteImpl(emojiID, api).setAnimated(animated).setName(name);
  ...
  public MessageEmbed createMessageEmbed(JSONObject content) {
    ...
    final OffsetDateTime timestamp =
        content.isNull("timestamp") ? null : OffsetDateTime.parse(content.getString("timestamp"));
    ...
    if (content.isNull("provider")) {
      ...
      provider = new Provider(obj.optString("name", null), obj.optString("url", null));
    }
  ...
  public PermissionOverride createPermissionOverride(JSONObject override, Channel chan) {
    ...
    // Throwing NoSuchElementException for common issues with overrides that are not cleared
    // properly by discord
      ...
      case "member":
        ...
        if (member == null)
          throw new NoSuchElementException(
              "Attempted to create a PermissionOverride for a non-existent user. Guild: "
                  + chan.getGuild()
                  + ", Channel: "
                  + chan
                  + ", JSON: "
        ...
        if (permOverride == null) {
          ...
          ((AbstractChannelImpl<?>) chan)
              .getOverrideMap()
              .put(member.getUser().getIdLong(), permOverride);
        }
        break;
      case "role":
  ...
  public Webhook createWebhook(JSONObject object) {
    ...
    if (channel == null)
      throw new NullPointerException(
          String.format(
              "Tried to create Webhook for an un-cached TextChannel! WebhookId: %s ChannelId: %s GuildId: %s",
    ...
    return new WebhookImpl(channel, id)
        .setToken(token)
        .setOwner(channel.getGuild().getMember(owner))
        .setUser(defaultUser);
  }
  ...
  public Invite createInvite(JSONObject object) {
    ...
    final User inviter =
        object.has("inviter") ? this.createFakeUser(object.getJSONObject("inviter"), false) : null;
    ...
    final Invite.Guild guild =
        new InviteImpl.GuildImpl(guildId, guildIconId, guildName, guildSplashId);
  ...
  public ApplicationInfo createApplicationInfo(JSONObject object) {
    ...
    return new ApplicationInfoImpl(
        api, description, doesBotRequireCodeGrant, iconId, id, isBotPublic, name, owner);
  }
  ...
  public AuditLogEntry createAuditLogEntry(
      GuildImpl guild, JSONObject entryJson, JSONObject userJson) {
    ...
    final JSONArray changes =
        entryJson.isNull("changes") ? null : entryJson.getJSONArray("changes");
    final JSONObject options =
        entryJson.isNull("options") ? null : entryJson.getJSONObject("options");
    ...
    CaseInsensitiveMap<String, AuditLogChange> changeMap =
        new CaseInsensitiveMap<>(changeToMap(changesList));
  ...
  private Map<String, AuditLogChange> changeToMap(Set<AuditLogChange> changesList) {
    return changesList
        .stream()
        .collect(Collectors.toMap(AuditLogChange::getKey, UnaryOperator.identity()));
  }
