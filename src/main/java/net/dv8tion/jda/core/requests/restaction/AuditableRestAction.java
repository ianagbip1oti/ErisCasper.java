...
/**
 * Extension of RestAction to allow setting a reason, only available to accounts of {@link
 * net.dv8tion.jda.core.AccountType#BOT AccounType.BOT}
 *
...
public abstract class AuditableRestAction<T> extends RestAction<T> {
  ...
  /**
   * Applies the specified reason as audit-log reason field. <br>
   * When the provided reason is empty or {@code null} it will be treated as not set.
   *
   * <p>Reasons for any AuditableRestAction may be retrieved via {@link
   * net.dv8tion.jda.core.audit.AuditLogEntry#getReason() AuditLogEntry.getReason()} in iterable
   * {@link net.dv8tion.jda.core.requests.restaction.pagination.AuditLogPaginationAction
   * AuditLogPaginationActions} from {@link net.dv8tion.jda.core.entities.Guild#getAuditLogs()
   * Guild.getAuditLogs()}!
   *
   * <p>This will specify the reason via the {@code X-Audit-Log-Reason} Request Header. <br>
   * <b>Note: This may not be available to accounts for {@link
   * net.dv8tion.jda.core.AccountType#CLIENT AccountType.CLIENT}. <br>
   * Using methods with a reason parameter will always work and <u>override</u> this header.</b>
   * (ct. {@link
   * net.dv8tion.jda.core.managers.GuildController#ban(net.dv8tion.jda.core.entities.User, int,
   * String) GuildController.ban(User, int, String)})
   *
  ...
  protected CaseInsensitiveMap<String, String> finalizeHeaders() {
    ...
    if (reason == null || reason.isEmpty()) return headers;
  ...
  /**
   * Specialized form of {@link net.dv8tion.jda.core.requests.restaction.AuditableRestAction
   * AuditableRestAction} that is used to provide information that has already been retrieved or
   * generated so that another request does not need to be made to Discord. <br>
  ...
  public static class EmptyRestAction<T> extends AuditableRestAction<T> {
    ...
    public void queue(Consumer<T> success, Consumer<Throwable> failure) {
      if (success != null) success.accept(content);
    }
