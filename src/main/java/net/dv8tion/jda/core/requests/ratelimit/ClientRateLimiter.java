    ...
    synchronized (bucket) {
      ...
      if (code == 429) {
        try (InputStream in = Requester.getBody(response)) {
          ...
          else bucket.retryAfter = now + retryAfter;
  ...
  private class Bucket implements IBucket, Runnable {
    ...
    Long getRateLimit() {
      ...
      if (globalCooldown != null) // Are we on global cooldown?
      {
        ...
        {
          globalCooldown =
              null; // If we are done cooling down, reset the globalCooldown and continue.
        } else {
          return globalCooldown
              - now; // If we should still be on cooldown, return when we can go again.
        }
      }
