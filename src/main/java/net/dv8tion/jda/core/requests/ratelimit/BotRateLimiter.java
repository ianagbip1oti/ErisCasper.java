...
public class BotRateLimiter extends RateLimiter {
  ...
  protected Long handleResponse(Route.CompiledRoute route, okhttp3.Response response) {
    ...
    synchronized (bucket) {
      ...
      if (timeOffset == null) setTimeOffset(headers);
  ...
  private void setTimeOffset(Headers headers) {
    ...
    if (timeOffset == null) {
      ...
      if (date != null) {
        ...
        long lDate =
            tDate.toInstant().toEpochMilli(); // We want to work in milliseconds, not seconds
  ...
  private void updateBucket(Bucket bucket, Headers headers) {
    try {
      if (bucket.hasRatelimit()) // Check if there's a hardcoded rate limit
      {
        ...
        bucket.resetTime =
            Long.parseLong(headers.get("X-RateLimit-Reset")) * 1000; // Seconds to milliseconds
        bucket.routeUsageLimit = Integer.parseInt(headers.get("X-RateLimit-Limit"));
      }

      // Currently, we check the remaining amount even for hardcoded ratelimits just to further
      // respect Discord
      // however, if there should ever be a case where Discord informs that the remaining is less
      // than what
      // it actually is and we add a custom ratelimit to handle that, we will need to instead move
      // this to the
      // above else statement and add a bucket.routeUsageRemaining-- decrement to the above if body.
      // An example of this statement needing to be moved would be if the custom ratelimit reset
      // time interval is
      // equal to or greater than 1000ms, and the remaining count provided by discord is less than
      // the ACTUAL
      // amount that their systems allow in such a way that isn't a bug.
      // The custom ratelimit system is primarily for ratelimits that can't be properly represented
      // by Discord's
      // header system due to their headers only supporting accuracy to the second. The custom
      // ratelimit system
      // allows for hardcoded ratelimits that allow accuracy to the millisecond which is important
      // for some
      // ratelimits like Reactions which is 1/0.25s, but discord reports the ratelimit as 1/1s with
      // headers.
      ...
      if (!bucket.getRoute().equals("gateway") && !bucket.getRoute().equals("users/@me")) {
        Requester.LOG.debug(
            "Encountered issue with headers when updating a bucket\nRoute: {}\nHeaders: {}",
            bucket.getRoute(),
            headers);
      }
    }
  ...
  private class Bucket implements IBucket, Runnable {
    ...
    volatile int routeUsageRemaining =
        1; // These are default values to only allow 1 request until we have properly
    ...
    Long getRateLimit() {
      ...
      if (this.routeUsageRemaining > 0) return null;
      else return this.resetTime - getNow();
    }
      ...
      try {
        synchronized (requests) {
          for (Iterator<Request> it = requests.iterator(); it.hasNext(); ) {
            ...
            if (limit != null && limit > 0) break; // possible global cooldown here
            Request request = null;
            try {
              request = it.next();
              if (isSkipped(it, request)) continue;
              ...
              it.remove();
              if (request != null) request.onFailure(t);
            }
          }
        ...
        Requester.LOG.error(
            "Requester system encountered an internal error from beyond the synchronized execution blocks. NOT GOOD!",
            err);
