...
/**
 * Constants representing {@link net.dv8tion.jda.core.requests.WebSocketClient WebSocketClient}
 * close codes with association to an explaining message. <br>
 * This was inspired from the <a target="_blank"
 * href="https://discordapp.com/developers/docs/topics/gateway#disconnections">official
 * documentation</a>
 */
public enum CloseCode {
  ...
  INTERNAL_SERVER_ERROR(
      1006, "Something broke on the remote's end, sorry 'bout that... Try reconnecting!"),
  ...
  INVALID_SEQ(
      4007,
      "The sent sent when resuming the session was invalid. Reconnect and start a new session."),
  ...
  /**
   * Whether the {@link net.dv8tion.jda.core.requests.WebSocketClient WebSocketClient} will attempt
   * to reconnect when this close code appears
   *
   ...
   * @return The CloseCode field matching the specified integer or {@code null} if no match was
   *     found
   */
  public static CloseCode from(int code) {
    for (CloseCode c : values()) if (c.code == code) return c;
    return null;
  }
}
