...
public class Response implements Closeable {
  ...
  protected Response(
      final okhttp3.Response response, final Exception exception, final Set<String> cfRays) {
  ...
  protected Response(
      final okhttp3.Response response, final long retryAfter, final Set<String> cfRays) {
  ...
  private <T> T parseBody(Class<T> clazz, Function<InputStream, T> parser) {
    if (attemptedParsing) {
      if (object != null && clazz.isAssignableFrom(object.getClass())) return clazz.cast(object);
      return null;
    }

    attemptedParsing = true;
    if (body == null || rawResponse == null || rawResponse.body().contentLength() == 0) return null;
