...
public class WebSocketClient extends WebSocketAdapter implements WebSocketListener {
  ...
  protected void updateTraces(JSONArray arr, String type, int opCode) {
    ...
    traces.clear();
    for (Object o : arr) traces.add(String.valueOf(o));
  }
  ...
  protected void extendBuffer(byte[] binary) throws IOException {
    if (this.readBuffer != null) this.readBuffer.write(binary);
  }
  ...
  private boolean send(String message, boolean skipQueue) {
    ...
    if (this.messagesSent <= 115
        || (skipQueue
            && this.messagesSent
                <= 119)) // technically we could go to 120, but we aren't going to chance it
    {
      LOG.trace("<- {}", message);
      ...
      if (!printedRateLimitMessage) {
        LOG.warn(
            "Hit the WebSocket RateLimit! If you see this message a lot then you might need to talk to DV8FromTheWorld.");
  ...
  private void setupSendingThread() {
            ...
            () -> {
              ...
              while (!Thread.currentThread().isInterrupted()) {
                try {
                  ...
                  if (audioRequest == null || chunkOrSyncRequest != null) maybeUnlock();
                  if (chunkOrSyncRequest != null) {
                    needRatelimit = !send(chunkOrSyncRequest, false);
                    if (!needRatelimit) chunkSyncQueue.removeFirst();
                    ...
                    if (!needRatelimit) {
                      // If we didn't get RateLimited, Next request attempt will be 2 seconds from
                      // now
                      // we remove it in VoiceStateUpdateHandler once we hear that it has updated
                      // our status
                    ...
                    if (message != null) {
                      ...
                      if (!needRatelimit) ratelimitQueue.removeFirst();
                      attemptedToSend = true;
                    }
                  }

                  if (needRatelimit || !attemptedToSend) Thread.sleep(1000);
    ...
    ratelimitThread.setUncaughtExceptionHandler(
        (thread, throwable) -> {
  ...
  public void close(int code) {
    if (socket != null) socket.sendClose(code);
  }
  ...
  */
  ...
  protected synchronized void connect() {
    ...
    String url =
        api.getGatewayUrl() + "?encoding=json&compress=zlib-stream&v=" + DISCORD_GATEWAY_VERSION;

    try {
      socket =
          api.getWebSocketFactory()
              .createSocket(url)
  ...
  protected String getGateway() {
    try {
      return api.getSessionController().getGateway(api)
          + "?encoding=json&compress=zlib-stream&v="
  ...
  public void onConnected(WebSocket websocket, Map<String, List<String>> headers) {
    ...
    if (sessionId == null) sendIdentify();
    else sendResume();
  }
  ...
  public void onDisconnected(
    ...
    if (serverCloseFrame != null) {
      ...
      else if (closeCode != null) LOG.debug("WebSocket connection closed with code {}", closeCode);
      else
        LOG.warn(
            "WebSocket connection closed with unknown meaning for close-code {}", rawCloseCode);
    }
    if (clientCloseFrame != null
      ...
      // in that case we can be sure that resume will not work and instead we invalidate and
      // reconnect here
      isInvalidate = true;
    }
    ...
    {
      if (ratelimitThread != null) ratelimitThread.interrupt();
      ...
      if (!closeCodeIsReconnect) {
        ...
        // if that is the case we have to drop our connection and inform the user with a fatal error
        // message
        LOG.error(
      ...
      readBuffer = null;
      if (isInvalidate) invalidate(); // 1000 means our session is dropped so we cannot resume
      api.getEventManager()
          .handle(
              new DisconnectEvent(
                  api, serverCloseFrame, clientCloseFrame, closedByServer, OffsetDateTime.now()));
      try {
        if (sessionId == null) queueReconnect();
  ...
  protected void queueReconnect() {
    if (!handleIdentifyRateLimit)
      LOG.warn(
          "Got disconnected from WebSocket (Internet?!)... Appending session to reconnect queue");
    try {
  ...
  /**
   * This method is used to start the reconnect of the JDA instance. It is public for access from
   * SessionReconnectQueue extensions.
   *
   * @param callFromQueue whether this was in SessionReconnectQueue and got polled
   * @param shouldHandleIdentify whether SessionReconnectQueue already handled an IDENTIFY rate
  ...
  public void reconnect(boolean callFromQueue, boolean shouldHandleIdentify)
    ...
    while (shouldReconnect) {
      ...
      if (handleIdentifyRateLimit && shouldHandleIdentify)
        LOG.error(
            "Encountered IDENTIFY (OP {}) Rate Limit! Waiting {} seconds before trying again!",
            WebSocketCode.IDENTIFY,
            IDENTIFY_DELAY);
      else delay = reconnectTimeoutS;
  ...
  protected void setupKeepAlive(long timeout) {
    ...
    keepAliveThread.setUncaughtExceptionHandler(
        (thread, throwable) -> {
          handleCallbackError(socket, throwable);
  ...
  protected void sendKeepAlive() {
    ...
    if (!send(keepAlivePacket, true)) ratelimitQueue.addLast(keepAlivePacket);
  ...
  protected void sendIdentify() {
    ...
    JSONObject payload =
        new JSONObject()
            ...
            .put("compress", true);
    JSONObject identify = new JSONObject().put("op", WebSocketCode.IDENTIFY).put("d", payload);
    if (shardInfo != null) {
      payload.put(
          "shard", new JSONArray().put(shardInfo.getShardId()).put(shardInfo.getShardTotal()));
    }
  ...
  protected void updateAudioManagerReferences() {
    ...
    if (managerMap.size() > 0) LOG.trace("Updating AudioManager references");
    ...
    synchronized (managerMap) {
      for (TLongObjectIterator<AudioManager> it = managerMap.iterator(); it.hasNext(); ) {
        ...
        if (guild == null) {
          // We no longer have access to the guild that this audio manager was for. Set the value to
          // null.
          ...
          if (mng.isConnected() || mng.isAttemptingToConnect()) {
            ...
            if (channel != null) {
              if (mng.isConnected()) mng.closeAudioConnection(ConnectionStatus.ERROR_CANNOT_RESUME);
  ...
  protected void handleEvent(JSONObject raw) {
    ...
    // If initiating, only allows READY, RESUMED, GUILD_MEMBERS_CHUNK, GUILD_SYNC, and GUILD_CREATE
    // through.
    ...
    if (initiating
      ...
      if (!isJSON) {
        if (type.equals("PRESENCES_REPLACE")) {
          List<JSONObject> converted =
              convertPresencesReplace(responseTotal, raw.getJSONArray("d"));
      ...
      // If we are currently GuildStreaming, and we get a GUILD_DELETE informing us that a Guild is
      // unavailable
      // convert it to a GUILD_CREATE for handling.
      JSONObject content = raw.getJSONObject("d");
      if (!chunkingAndSyncing
          && type.equals("GUILD_DELETE")
          && content.has("unavailable")
    ...
    if (!isJSON) {
      ...
      if (type.equals("PRESENCES_REPLACE")) {
        ...
        for (JSONObject o : converted) handler.handle(responseTotal, o);
      } else {
    ...
    try {
      switch (type) {
        ...
        default:
          ...
          else LOG.debug("Unrecognized event:\n{}", raw);
      }
      ...
      LOG.error(
          "Got an unexpected error. Please redirect following message to the devs:\n\t{} -> {}",
          type,
          content,
          ex);
    }
  ...
  protected boolean onBufferMessage(byte[] binary) throws IOException {
    ...
    if (readBuffer != null) extendBuffer(binary);
    else allocateBuffer(binary);
  ...
  public void onBinaryMessage(WebSocket websocket, byte[] binary)
    ...
    try (InflaterOutputStream decompressor = new InflaterOutputStream(out, zlibContext)) {
      if (readBuffer != null) readBuffer.writeTo(decompressor);
      else decompressor.write(binary);
  ...
  private void maybeUnlock() {
    if (audioQueueLock.isHeldByCurrentThread()) audioQueueLock.unlock();
  }
  ...
  protected class StartingNode extends ConnectNode {
    ...
    public void run(boolean isLast) throws InterruptedException {
      ...
      connect();
      while (!isLast
          && api.getStatus().ordinal() < JDA.Status.AWAITING_LOGIN_CONFIRMATION.ordinal())
        Thread.sleep(50);
    }
  }
