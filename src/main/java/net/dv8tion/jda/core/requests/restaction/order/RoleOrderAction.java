 ...
 * designed to modify the order of {@link net.dv8tion.jda.core.entities.Role Roles} of the specified
 ...
 * #selectPosition(Object) selectPosition(Role)} or {@link #selectPosition(int)}!
 *
...
public class RoleOrderAction extends OrderAction<Role, RoleOrderAction> {
   ...
   *
   * @param guild The target {@link net.dv8tion.jda.core.entities.Guild Guild} of which to change
   *     the {@link net.dv8tion.jda.core.entities.Role Role} order
  ...
  public RoleOrderAction(Guild guild, boolean useDiscordOrder) {
    ...
    if (useDiscordOrder) {
      ...
      // We add first to last because the roles provided from getRoles() are in ascending order
      // already
  ...
  /**
   * The {@link net.dv8tion.jda.core.entities.Guild Guild} which holds the roles from {@link
   * #getCurrentOrder()}
   *
  ...
  protected RequestBody finalizeData() {
    ...
    for (int i = 0; i < ordering.size(); i++) {
      ...
      if (initialPos != i && !isOwner && !self.canInteract(role))
        // If the current role was moved, we are not owner and we can't interact with the role then
