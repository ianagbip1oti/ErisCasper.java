...
/**
 * This class serves as a LoggerFactory for JDA's internals. <br>
 * It will either return a Logger from a SLF4J implementation via {@link org.slf4j.LoggerFactory} if
 * present, or an instance of a custom {@link SimpleLogger} (From slf4j-simple).
 *
 * <p>It also has the utility method {@link #getLazyString(LazyEvaluation)} which is used to lazily
 * construct Strings for Logging.
 */
public class JDALogger {
  /**
   * Marks whether or not a SLF4J <code>StaticLoggerBinder</code> was found. If false, JDA will use
   * its fallback logger. <br>
  ...
  /**
   * Will get the {@link org.slf4j.Logger} with the given log-name or create and cache a fallback
   * logger if there is no SLF4J implementation present.
   *
   * <p>The fallback logger will be an instance of a slightly modified version of SLF4Js
   * SimpleLogger.
   *
  ...
  public static Logger getLog(String name) {
    synchronized (LOGS) {
      if (SLF4J_ENABLED) return LoggerFactory.getLogger(name);
      return LOGS.computeIfAbsent(name, SimpleLogger::new);
    }
  }

  /**
   * Will get the {@link org.slf4j.Logger} for the given Class or create and cache a fallback logger
   * if there is no SLF4J implementation present.
   *
   * <p>The fallback logger will be an instance of a slightly modified version of SLF4Js
   * SimpleLogger.
   *
  ...
  /**
   * Functional interface used for {@link #getLazyString(LazyEvaluation)} to lazily construct a
   * String.
   */
  @FunctionalInterface
  public interface LazyEvaluation {
    /**
     * This method is used by {@link #getLazyString(LazyEvaluation)} when SLF4J requests String
     * construction. <br>
