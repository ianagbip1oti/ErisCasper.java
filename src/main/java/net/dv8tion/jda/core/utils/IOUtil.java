...
public class IOUtil {
   ...
   *
   * <p>This customized version for File is provide (instead of just using {@link
   * #readFully(java.io.InputStream)} with a FileInputStream) because with a File we can determine
   * the total size of the array and do not need to have a buffer. This results in a memory
   * footprint that is half the size of {@link #readFully(java.io.InputStream)}
   *
   ...
   *
   * <p>This method will block until the InputStream has been fully read, so if you provide an
   * InputStream that is non-finite, you're gonna have a bad time.
   *
