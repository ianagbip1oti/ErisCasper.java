...
public class MemberCacheViewImpl extends AbstractCacheView<Member> implements MemberCacheView {
  ...
  public List<Member> getElementsByUsername(String name, boolean ignoreCase) {
    ...
    for (Member member : elements.valueCollection()) {
      ...
      if (ignoreCase) {
        if (nick.equalsIgnoreCase(name)) members.add(member);
      } else {
        if (nick.equals(name)) members.add(member);
      }
    }
  ...
  public List<Member> getElementsWithRoles(Role... roles) {
    ...
    for (Role role : roles) Checks.notNull(role, "Roles");
    List<Member> members = new LinkedList<>();
    memberLoop:
    for (Member member : elements.valueCollection()) {
      for (Role role : roles) {
        if (!member.getRoles().contains(role)) continue memberLoop;
      }
      members.add(member);
    }
    return members;
  ...
  public List<Member> getElementsWithRoles(Collection<Role> roles) {
    ...
    List<Member> members = new LinkedList<>();
    for (Member member : elements.valueCollection()) {
      if (member.getRoles().containsAll(roles)) members.add(member);
    }
    return members;
  }
}
