...
/**
 * Represents the contents of a audio packet that was either received from Discord or will be sent
 * to discord.
 */
public class AudioPacket {
  ...
  /**
   * Bit index 0 and 1 represent the RTP Protocol version used. Discord uses the latest RTP protocol
   * version, 2.<br>
   * Bit index 2 represents whether or not we pad. Opus uses an internal padding system, so RTP
   * padding is not used.<br>
   * Bit index 3 represents if we use extensions. Discord does not use RTP extensions.<br>
   * Bit index 4 to 7 represent the CC or CSRC count. CSRC is Combined SSRC. Discord doesn't combine
   * audio streams, so the Combined count will always be 0 (binary: 0000).<br>
  ...
  public AudioPacket(byte[] rawPacket) {
    ...
    if ((versionPad & 0b0001_0000) != 0
        && data[RTP_HEADER_BYTE_LENGTH] == (byte) 0xBE
        && data[RTP_HEADER_BYTE_LENGTH + 1] == (byte) 0xDE) {
      final short headerLength =
          (short) (data[RTP_HEADER_BYTE_LENGTH + 2] << 8 | data[RTP_HEADER_BYTE_LENGTH + 3]);
      ...
      System.arraycopy(
          buffer.array(), RTP_HEADER_BYTE_LENGTH, this.encodedAudio, 0, this.encodedAudio.length);
    }
  }
  ...
  public AudioPacket(char seq, int timestamp, int ssrc, byte[] encodedAudio) {
    ...
    System.arraycopy(
        encodedAudio, 0, buffer.array(), RTP_HEADER_BYTE_LENGTH, encodedAudio.length); // 12 - n
  ...
  public DatagramPacket asUdpPacket(InetSocketAddress address) {
    // We use getRawPacket() instead of the rawPacket variable so that we get a copy of the array
    // instead of the
  ...
  public static AudioPacket decryptAudioPacket(DatagramPacket packet, byte[] secretKey) {
    ...
    System.arraycopy(
        decryptedAudio, 0, decryptedRawPacket, RTP_HEADER_BYTE_LENGTH, decryptedAudio.length);
