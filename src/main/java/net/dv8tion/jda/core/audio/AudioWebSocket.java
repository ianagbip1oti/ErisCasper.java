...
public class AudioWebSocket extends WebSocketAdapter {
  ...
  public AudioWebSocket(
    ...
    if (sessionId == null || sessionId.isEmpty())
      throw new IllegalArgumentException(
          "Cannot create a voice connection using a null/empty sessionId!");
    if (token == null || token.isEmpty())
      throw new IllegalArgumentException(
          "Cannot create a voice connection using a null/empty token!");
  }
  ...
  protected void send(int op, Object data) {
    send(new JSONObject().put("op", op).put("d", data == null ? JSONObject.NULL : data).toString());
  }
  ...
  public void onConnected(WebSocket websocket, Map<String, List<String>> headers) {
    ...
    if (!reconnecting) audioConnection.ready();
  }
      ...
      case VoiceCode.HELLO:
        {
          ...
          // FIXME: discord will rollout a working interval once that is done we need to use it
          // properly
          break;
        }
      ...
      case VoiceCode.SESSION_DESCRIPTION:
        {
          ...
          for (int i = 0; i < keyArray.length(); i++) secretKey[i] = (byte) keyArray.getInt(i);
      ...
      case VoiceCode.USER_SPEAKING_UPDATE:
        {
          ...
          if (user == null) {
            ...
            AudioConnection.LOG.trace(
                "Got an Audio USER_SPEAKING_UPDATE for a non-existent User. JSON: {}", contentAll);
            break;
          }
  ...
  public void startConnection() {
    ...
    try {
      socket = api.getWebSocketFactory().createSocket(wssEndpoint).addListener(this);
      changeStatus(ConnectionStatus.CONNECTING_AWAITING_WEBSOCKET_CONNECT);
      socket.connectAsynchronously();
    } catch (IOException e) {
      LOG.warn(
          "Encountered IOException while attempting to connect: {}\nClosing connection and attempting to reconnect.",
          e.getMessage());
      this.close(ConnectionStatus.ERROR_WEBSOCKET_UNABLE_TO_CONNECT);
    }
  }
  ...
  public void close(final ConnectionStatus closeStatus) {
    ...
    locked(
        (manager) -> {
          ...
          if (socket != null && socket.isOpen()) socket.sendClose(1000);

          VoiceChannel disconnectedChannel;
          if (audioConnection != null) audioConnection.shutdown();

          if (manager.getConnectedChannel() != null)
            disconnectedChannel = manager.getConnectedChannel();
          else disconnectedChannel = manager.getQueuedAudioConnection();

          manager.setAudioConnection(null);

          // Verify that it is actually a lost of connection and not due the connected channel being
          // deleted.
          ...
          if (shouldReconnect
              && status
                  != ConnectionStatus
                      .NOT_CONNECTED // indicated that the connection was purposely closed. don't
              // reconnect.
              && status != ConnectionStatus.DISCONNECTED_CHANNEL_DELETED
  ...
  private InetSocketAddress handleUdpDiscovery(InetSocketAddress address, int ssrc) {
    ...
    try {
      ...
      ByteBuffer buffer = ByteBuffer.allocate(70); // 70 taken from
      // https://github.com/Rapptz/discord.py/blob/async/discord/voice_client.py#L208
      buffer.putInt(
          ssrc); // Put the ssrc that we were given into the packet to send back to discord.
      ...
      // Discord responds to our packet, returning a packet containing our external ip and the port
      // we connected through.
      DatagramPacket receivedPacket =
          new DatagramPacket(new byte[70], 70); // Give a buffer the same size as the one we sent.
      ...
      String ourIP =
          new String(
              receivedPacket
                  .getData()); // Puts the entire byte array in. nulls are converted to spaces.
      ourIP =
          ourIP.substring(
              4,
              ourIP.length()
                  - 2); // Removes the SSRC of the answer package and the port that is stuck on the
      // end of this string. (last 2 bytes are the port)
      ...
      // We will first need to convert the byte order from Little Endian to Big Endian (reverse the
      // order)
      // Then we will need to deal with the fact that the bytes represent an unsigned short.
      // Java cannot deal with unsigned types, so we will have to promote the short to a higher
      // type.
      ...
      int firstByte =
          (0x000000FF
              & ((int) portBytes[0])); // Promotes to int and handles the fact that it was unsigned.
