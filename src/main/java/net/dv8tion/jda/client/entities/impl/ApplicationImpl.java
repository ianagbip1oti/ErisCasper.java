...
public class ApplicationImpl implements Application {
  ...
  public RestAction<Application.Bot> createBot() {
    if (this.hasBot()) return new RestAction.EmptyRestAction<>(getJDA(), this.bot);

    return new RestAction<Application.Bot>(
        this.api, Route.Applications.CREATE_BOT.compile(getId())) {
      @Override
      protected void handleResponse(
          final Response response, final Request<Application.Bot> request) {
        if (response.isOk())
  ...
  public ApplicationManager getManager() {
    ...
    if (mng == null) {
      synchronized (mngLock) {
        mng = manager;
        if (mng == null) mng = manager = new ApplicationManager(this);
      }
    }
    return mng;
  }
  ...
  public ApplicationManagerUpdatable getManagerUpdatable() {
    ...
    if (mng == null) {
      synchronized (mngLock) {
        ...
        if (mng == null) mng = managerUpdatable = new ApplicationManagerUpdatable(this);
      }
    }
    return mng;
  }
  ...
  public ApplicationImpl updateFromJson(final JSONObject object) {
    if (object.has("bot")) {
      ...
      if (this.bot == null) this.bot = new BotImpl(botObject);
      else this.bot.updateFromJson(botObject);
    } else {
      this.bot = null;
    }
    ...
    if (this.redirectUris == null) this.redirectUris = new ArrayList<>(redirectUriArray.length());
    else this.redirectUris.clear();
  ...
  public class BotImpl implements Application.Bot {
    ...
    public String getInviteUrl(final String guildId, final Collection<Permission> permissions) {
      StringBuilder builder =
          new StringBuilder("https://discordapp.com/oauth2/authorize?client_id=");
    ...
    public RestAction<Bot> resetToken() {
      ...
      return new RestAction<Bot>(getJDA(), route) {
        ...
        protected void handleResponse(final Response response, final Request<Bot> request) {
          if (response.isOk()) request.onSuccess(BotImpl.this.updateFromJson(response.getObject()));
          else request.onFailure(response);
        }
      };
    }
