  ...
  protected Long handleInternally(JSONObject content) {
    ...
    if (channel == null) {
      ...
      EventCache.LOG.debug(
          "Received a CALL_CREATE for a Group/PrivateChannel that is not yet cached. JSON: {}",
          content);
      return null;
    }
    ...
    if (channel instanceof Group) {
      ...
      if (group.getCurrentCall() != null)
        WebSocketClient.LOG.error(
            "Received a CALL_CREATE for a Group that already has an active call cached! JSON: {}",
            content);
      ...
      if (priv.getCurrentCall() != null)
        WebSocketClient.LOG.error(
            "Received a CALL_CREATE for a PrivateChannel that already has an active call cached! JSON: {}",
            content);
