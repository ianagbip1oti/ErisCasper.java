...
public class CallUpdateHandler extends SocketHandler {
  ...
  protected Long handleInternally(JSONObject content) {
    ...
    if (channel == null) {
      ...
      EventCache.LOG.debug(
          "Received a CALL_UPDATE for a Group/PrivateChannel that has not yet been cached. JSON: {}",
          content);
      return null;
    }
    ...
    if (call == null) {
      ...
      EventCache.LOG.debug(
          "Received a CALL_UPDATE for a Call that has not yet been cached. JSON: {}", content);
      return null;
    }
    ...
    if (ringing.length() > 0) {
      ...
      // Any Ids that are users that have started ringing, so we need to set their ringing status as
      // such
        ...
        api.getEventManager()
            .handle(
                new CallUpdateRingingUsersEvent(
                    api, responseNumber, call, stoppedRingingUsers, startedRingingUsers));
      }
    }
    return null;
  }
  ...
  private List<Long> toLongList(JSONArray array) {
    ...
    for (int i = 0; i < array.length(); i++) longs.add(array.getLong(i));
