...
public class RelationshipRemoveHandler extends SocketHandler {
  ...
  protected Long handleInternally(JSONObject content) {
    ...
    if (type == RelationshipType.NO_RELATIONSHIP) return null;

    // Make sure that we get the proper relationship, not just any one cached by this userId.
    // Deals with possibly out of order RELATIONSHIP_REMOVE and RELATIONSHIP_ADD when blocking a
    // Friend.
    Relationship relationship = api.asClient().getRelationshipById(userId, type);
    if (relationship == null) {
      api.getEventCache()
          .cache(EventCache.Type.RELATIONSHIP, userId, () -> handle(responseNumber, allContent));
      EventCache.LOG.debug(
          "Received a RELATIONSHIP_REMOVE for a relationship that was not yet cached! JSON: {}",
          content);
      return null;
    }
    ...
    if (relationship.getType() == RelationshipType.FRIEND) {
      ...
      // Note: we getGroups() which gets all groups, however we already removed the user from the
      // current group.
    ...
    switch (type) {
      case FRIEND:
        api.getEventManager().handle(new FriendRemovedEvent(api, responseNumber, relationship));
        break;
      case BLOCKED:
        api.getEventManager().handle(new UserUnblockedEvent(api, responseNumber, relationship));
        break;
      case INCOMING_FRIEND_REQUEST:
        api.getEventManager()
            .handle(new FriendRequestIgnoredEvent(api, responseNumber, relationship));
        break;
      case OUTGOING_FRIEND_REQUEST:
        api.getEventManager()
            .handle(new FriendRequestCanceledEvent(api, responseNumber, relationship));
        break;
      default:
        WebSocketClient.LOG.warn(
            "Received a RELATIONSHIP_REMOVE with an unknown RelationshipType! JSON: {}", content);
        return null;
    }
